...
- **Durable, persistent queue** -- Accepted emails are written to a local SQLite outbox *before* the SMTP `250 OK` and only removed once delivered, so messages survive process restarts and crashes (delivery is *at-least-once*: a crash between a successful send and the queue removal may cause a redelivery, which is deduplicated via a stable Matrix transaction id within a session). The outbox is size-bounded (`queue.max_len`); when full, new mail is refused with a temporary `451` so senders retry.
